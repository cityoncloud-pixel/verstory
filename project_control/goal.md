...
11) 项目管理可用：项目支持改名；删除项目稳定可用（不出现浏览器 `Failed to fetch`）；删除后该项目下录音与文本不再可见。
12) 录音删除可用：单条删除与“管理录音→批量删除”都稳定可用（不出现浏览器 `Failed to fetch`），删除后列表即时刷新。
...
- 录音单次时长限制：默认上限 15 分钟，并在录音界面明确提示剩余/上限（避免误以为可无限录制）。
